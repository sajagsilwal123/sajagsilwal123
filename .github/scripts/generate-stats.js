...
const { getPublicGitHubStats } = require('../../lib/github/stats');
 ...
 * Aggregates public + private contributions, active days, streaks, and repository counts.
 */
function generateMetricsRow(data) {
  const totalRepos = data.totalRepositories || (data.publicRepositories + data.privateRepositories);
  const svg = `
<svg width="800" height="90" viewBox="0 0 800 90" xmlns="http://www.w3.org/2000/svg">
  ...
  <g transform="translate(10, 40)">
    <text x="0" y="0" class="val">${formatNum(data.totalContributions)}</text>
  ...
  <g transform="translate(610, 40)">
    <text x="0" y="0" class="val">${totalRepos}</text>
    <text x="0" y="24" class="label">repositories</text>
    <text x="0" y="44" class="sub">${data.publicRepositories} public · ${data.privateRepositories} private</text>
  </g>
</svg>
  `;
 ...
 * Displays years on GitHub, total lifetime commits, PRs merged, issues closed, reviews, and stars.
 */
...
<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">
  ...
  <g transform="translate(25, 87)">
    ...
    <text x="${w-50}" y="11" class="text" text-anchor="end" font-weight="600">${data.totalCommits}</text>
  </g>
  ...
  <g transform="translate(25, 116)">
    ...
    <text x="${w-50}" y="11" class="text" text-anchor="end" font-weight="600">${data.pullRequestsMerged}</text>
  </g>
  ...
  <g transform="translate(25, 203)">
    ...
    <text x="${w-50}" y="11" class="text" text-anchor="end" font-weight="600">${data.starsEarned}</text>
  </g>
</svg>
  `;
 ...
 * Note: Metric is renamed to 'Most active public repo' to strictly prevent private repository name leaks.
 */
...
<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">
  ...
  <g transform="translate(25, 87)">
    ...
    <text x="${w-50}" y="11" class="text" text-anchor="end" font-weight="600">${data.mostActiveWeekday}</text>
  </g>
  
  <!-- Most active public repo (Safe from private leak) -->
  <g transform="translate(25, 116)">
    ...
    <text x="24" y="11" class="text">Most active public repo</text>
    <text x="${w-50}" y="11" class="text" text-anchor="end" font-weight="600">${data.mostActivePublicRepo}</text>
  </g>
...
function generateLanguagesBar(data) {
  ...
  const totalSize = (data.languages || []).reduce((acc, l) => acc + (l.size || 0), 0);
  ...
  const segments = (data.languages || []).map(l => {
    ...
    const pct = l.percentage || (frac * 100).toFixed(1);
    return { ...l, width, frac, percentage: pct };
  });
...
async function run() {
  console.log('[Dashboard] Executing authenticated stats generator across public & private repositories...');
  const data = await getPublicGitHubStats();
  
  console.log('[Dashboard] Generating SVGs with unified public + private metrics...');
  ...
  console.log('✓ All analytics SVGs updated successfully with public + private repository metrics.');
}
